...
import { getFileFormatInfo } from "@/shared/config/fileFormats"
import { getRawDimensions } from "@/shared/services/rawDecoder"

/**
 * Extract metadata dari image (EXIF) atau video (mediainfo.js).
 *
 * `deep` controls the accurate-but-expensive RAW dimension pass: for RAW files,
 * EXIF only exposes the embedded thumbnail's size (e.g. 160×120 for a Nikon
 * NEF), so we ask libraw for the true sensor dimensions. That spins up a wasm
 * worker, so the bulk folder scan calls with deep=false (it only needs capture
 * dates), and the on-demand pass for the viewed photo calls with deep=true.
 */
export const extractMetadata = async (file: File, deep = false): Promise<PhotoMetadata> => {
  const format = getFileFormatInfo(file.name)
  ...
  try {
    ...
    const meta = await extractImageMetadata(file, defaultMetadata)

    // RAW: EXIF gave us the thumbnail's size — replace it with libraw's real
    // sensor dimensions when we're allowed to do the expensive pass.
    if (deep && format?.category === "raw") {
      const dims = await getRawDimensions(file)
      if (dims) {
        return {
          ...meta,
          dimensions: dims,
          megapixels: megapixelsFrom(dims.width, dims.height)
        }
      }
    }
...
/** Megapixels rounded to 1 decimal place, or 0 when dimensions are unknown. */
const megapixelsFrom = (width: number, height: number): number =>
  width && height ? Math.round(((width * height) / 1_000_000) * 10) / 10 : 0
...
const extractImageMetadata = async (file: File, base: PhotoMetadata): Promise<PhotoMetadata> => {
  const slice = file.size > MAX_HEADER_BYTES ? file.slice(0, MAX_HEADER_BYTES) : file
  ...
  const megapixels = megapixelsFrom(width, height)

  return {
    ...base,
    cameraMake: desc(exif.Make),
    cameraModel: desc(exif.Model),
    lens: desc(exif.LensModel, exif.Lens, exif.LensSpecification, exif.LensMake),
    iso:
    ...
    dateTaken: desc(exif.DateTimeOriginal, exif.DateTimeDigitized, exif.DateTime),
    megapixels,
    dimensions: { width, height }
  }
}

const extractVideoMetadata = async (file: File, base: PhotoMetadata): Promise<PhotoMetadata> => {
  ...
  const duration = toNum(generalTrack?.Duration) ?? toNum(videoTrack?.Duration)
  const fps = toNum(videoTrack?.FrameRate)
  const bitrate = toNum(generalTrack?.OverallBitRate) ?? toNum(videoTrack?.BitRate)
...
export const formatFileSize = (bytes: number): string => {
  ...
  // Clamp so a huge (>= 1 PB) size can't index past the array and print "undefined".
