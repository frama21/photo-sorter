...
const DB_FILENAME = "nata-photo-db.json"
// Pre-rebrand database filename. Read as a fallback so users who sorted under the
// old name keep their project state; the next write re-saves it under DB_FILENAME.
const LEGACY_DB_FILENAME = "photo-sorter-db.json"
...
export const initProjectDatabase = async (dirHandle: FileSystemDirectoryHandle): Promise<CompleteProjectState> =>
  enqueue(async () => {
    ...
    if (existing) {
      addStatus({ type: "success", message: t("toast.projectLoaded"), icon: "db" })
      return existing
    }
...
export const updateDatabaseMode = async (
  dirHandle: FileSystemDirectoryHandle,
  moveMode: MoveMode
): Promise<void> => mutateDatabase(dirHandle, () => ({ moveMode }), t("toast.dbModeFailed"))
...
const loadProjectState = async (dirHandle: FileSystemDirectoryHandle): Promise<CompleteProjectState | null> => {
  ...
  try {
    ...
    // No current database — try the pre-rebrand filename before giving up, so
    // existing projects migrate seamlessly. Absent both, it's a first open.
    try {
      const legacyHandle = await dirHandle.getFileHandle(LEGACY_DB_FILENAME)
  ...
  if (parsed.version !== DB_VERSION) {
    addStatus({
      type: "error",
      message: t("toast.dbIncompatible", { version: String(parsed.version) }),
      icon: "db"
    })
