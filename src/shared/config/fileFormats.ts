...
// Derived lookups, internal to this module.
const ALL_EXTENSIONS: string[] = Object.values(PHOTO_FORMATS).flatMap(f => f.extensions)
const PREVIEWABLE_EXTENSIONS: string[] = Object.values(PHOTO_FORMATS)
  .filter(f => f.previewable)
  .flatMap(f => f.extensions)

const EXTENSION_TO_FORMAT: Record<string, FileFormatConfig> = {}
...
/** Lower-cased file extension including the leading dot, e.g. ".jpg". */
const getExtension = (filename: string): string => filename.toLowerCase().slice(filename.lastIndexOf("."))

export const isSupportedImage = (filename: string): boolean => ALL_EXTENSIONS.includes(getExtension(filename))

export const isPreviewable = (filename: string): boolean => PREVIEWABLE_EXTENSIONS.includes(getExtension(filename))

export const getFileFormatInfo = (filename: string): FileFormatConfig | null =>
  EXTENSION_TO_FORMAT[getExtension(filename)] ?? null
